...
def count_layout_screens(
    layout_name_or_path, dir=os.path.expanduser("~/.fluxbox/layouts")
):
    """
...
def get_layout_params(layout_file, use_cache=False, screens_only=False):
    ...
    for i, s in enumerate(screens):
        ...
        s["active_height"] = (
            s["height"] - s["top_margin"] - s["panel_height"] - s["logs_height"]
        )
        ...
        s["single_width"] = int(
            s["single_width_pc_of_active"] * s["active_width"] / 100
        )
        s["single_height"] = int(
            s["single_height_pc_of_active"] * s["active_height"] / 100
        )
        s["single_left"] = s["left_margin"] + int(
            (s["active_width"] - s["single_width"]) / 2
        )
        s["single_top"] = s["top_margin"] + int(
            (s["active_height"] - s["single_height"]) / 2
        )
    ...
    if not args.layout_name:
        ...
        try:
            result = subprocess.run(
                [get_layout_script], capture_output=True, text=True, check=True
            )
