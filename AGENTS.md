...
- **HiDPI support**: Scaling factors via `calculate_ui_scale_factor()` in libdpy.py; layout `ui_scale` overrides panel and window-manager scaling, while `font_scale` can override fonts independently
...
- **Font scaling**: `libfonts.sh` calculates font sizes per-host; layout `font_scale` overrides font sizing, falling back to `ui_scale` when absent
...
below. Work is NOT complete until they are completed.
